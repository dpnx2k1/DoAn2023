...
class product{
    ...
    public function show_category_byid($category_id){
    ...
    public function show_brand_byid($brand_id){
        $query="SELECT tbl_brand.*,tbl_category.category_name FROM tbl_brand INNER JOIN tbl_category ON
        tbl_brand.category_id=tbl_category.category_id WHERE brand_id=$brand_id ";
        $result = $this ->db->select($query);
        return $result;
    }
    ...
    public function insert_product(){
        ...
        $query = "INSERT INTO tbl_product(
                                        ...
                                        product_pre,
                                        product_img
        ) VALUES('$product_name','$category_id','$brand_id','$product_color_name','$product_size','$product_price','$product_total','$product_price_pro','$product_description','$product_pre','$product_img')";
    ...
    public function update_product(){
        ...
        $product_img=$_FILES['product_img']['name'];
        move_uploaded_file($_FILES['product_img']['tmp_name'],"Upload/".$_FILES['product_img']['name']);
         $query="UPDATE tbl_product SET product_name ='$product_name',category_id = '$category_id',brand_id='$brand_id',product_color_name='$product_color_name',
         product_size='$product_size',product_price=$product_price,product_total=$product_total,product_price_pro=$product_price_pro,product_description='$product_description',
         product_pre='$product_pre',product_img='$product_img' WHERE product_id='$product_id'";
        ...
        if ($result) {
            ...
            foreach($filename as $key => $value) {
                move_uploaded_file($filetmp[$key],"Upload/".$value);
                $query="UPDATE tbl_product_img_desciption SET product_img_desciption='$value' WHERE product_id='$product_id'";
                $result = $this ->db->update($query);
            }
        }
